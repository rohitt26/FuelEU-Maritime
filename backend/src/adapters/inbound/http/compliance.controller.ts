...
type ComplianceControllerDeps = {
  routeRepo?: RouteRepository;
  complianceRepo?: ComplianceRepository;
};

export function createComplianceController(
  deps: ComplianceControllerDeps = {}
): Router {
  const router = Router();
  const routeRepo = deps.routeRepo ?? new RouteRepositoryPg();
...
export default createComplianceController();
